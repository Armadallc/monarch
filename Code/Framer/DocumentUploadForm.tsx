...
const FORM_REDIRECT_URL = `${SITE_BASE}/submit-referrals/documents`
const PORTAL_DASHBOARD_URL = `${SITE_BASE}/portal`
...
const DOCUMENT_TYPE_OPTIONS = [
    ...
    { value: "competency_eval", label: "Competency Eval Report" },
...
export default function DocumentUploadForm() {
                ...
                if (updateError) {
                    ...
                    if (uploadedTypes.length > 0) {
                        const { error: syncErr } = await supabase.rpc(
                            "sync_document_request_items_on_upload",
                            {
                                p_referral_id: referralId,
                                p_document_types: uploadedTypes,
                            }
                        )
                        if (syncErr) {
                            console.warn(
                                "[DocumentUploadForm] sync document request items:",
                                syncErr
                            )
                        }
                    }
    ...
    // STYLES
    // ========================================================================
    const inputStyle: React.CSSProperties = { ...INPUT_BASE, width: "100%", boxSizing: "border-box" }
    ...
    /** Full-width shell: clears fixed site nav, centers content vertically on /submit-referrals/documents */
    const documentUploadPageShell: React.CSSProperties = {
        width: "100%",
        ...
        alignItems: "center",
        paddingTop: "clamp(132px, 20vh, 240px)",
            ...
            {step === "verify" && (
                <div>
                    <button
                        type="button"
                        onClick={() => {
                            try { window.location.href = `${SITE_BASE}/submit-referrals` } catch (_e) {}
                        }}
                        style={{
                            background: "none",
                            border: "none",
                            ...
                            {fileEntries.map((entry, index) => (
                                ...
                                >
                                    ...
                                    <select
                                        ...
                                        aria-label="Document type for this file"
                                        style={{
                                            width: "100%",
                                            padding: "8px",
                                            fontSize: "14px",
