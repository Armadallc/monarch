...
import {
    ...
    resolveLoginAuthBucket,
    writeAuthBucketHint,
} from "../lib/authGatewaySession"
import { startProviderOAuth } from "../lib/oauth"
import { createAuthGatewaySupabase, promoteStaffSessionToStaffBucket } from "../lib/supabaseAuth"
...
const SHADOWS = { modal: "0 24px 48px -12px rgba(43, 40, 40, 0.15)" }
...
function AuthLogotype() {
    return (
        <img
            src={DEFAULT_LOGOTYPE_URL}
    ...
    return (
        <div style={LOGIN_PAGE_SHELL}>
            <div style={{ ...LOGIN_CARD, textAlign: "center" }}>
                <AuthLogotype />
                <p style={{ color: COLORS.errorText, fontSize: "15px", lineHeight: 1.5 }}>
                    Supabase is not configured. Set <code>VITE_SUPABASE_URL</code> and{" "}
                    <code>VITE_SUPABASE_ANON_KEY</code> in Vercel environment variables, then redeploy.
                </p>
            </div>
        </div>
    )
...
export default function AuthGateway() {
    ...
    const [session, setSession] = useState<{ user?: { email?: string; phone?: string } } | null>(null)
    ...
    useEffect(() => {
        if (!supabase || switchHandledRef.current) return
    ...
    const handleMagicLink = async () => {
        ...
        const { error: err } = await supabase.auth.signInWithOtp({
            email,
            options: { emailRedirectTo: loginUrlForBucket(bucket) },
        })
        if (err) {
    ...
    if (session) {
        ...
        const phoneUser = session.user?.phone
                    ...
                    <div style={{ ...LOGIN_CARD, textAlign: "center" }}>
                        ...
                        <button
                            ...
                            onClick={async () => {
                                await supabase.auth.signOut({ scope: "global" })
                                setSession(null)
                                window.location.replace(`${referralPartnerLoginUrl()}&switch=1`)
                            }}
                ...
                <div style={LOGIN_PAGE_SHELL}>
                    <div style={{ ...LOGIN_CARD, textAlign: "center" }}>
                        ...
                        <button
                            ...
                            onClick={async () => {
                                await supabase.auth.signOut({ scope: "global" })
                                window.location.replace(`${staffLoginUrl()}&switch=1`)
                            }}
                ...
                <div style={{ ...LOGIN_CARD, textAlign: "center" }}>
                    ...
                    <button
                        ...
                        onClick={async () => {
                            const switchUrl =
                                authBucket === "staff"
                                    ? `${staffLoginUrl()}&switch=1`
                                    : `${referralPartnerLoginUrl()}&switch=1`
                            await supabase.auth.signOut({ scope: "global" })
